...
void StockData::fetch(const QString &symbol)
{
    ...
    const QString url = QStringLiteral("https://query2.finance.yahoo.com/v8/finance/chart/%1?range=1d&interval=1d").arg(symbol);
        ...
        [this, reply, symbol]()
            {
                ...
                if(reply->error() == QNetworkReply::NoError)
                {
                    ...
                    if (result)
                    {
                        ...
                        qDebug() << "Fetched stock data for" << quote.symbol << ":" << quote.price << quote.currency << quote.regularMarketTime;
                    
                        this->m_resultText = QStringLiteral("The price is %1 %2 at %3").arg(quote.price).arg(quote.currency).arg(QDateTime::fromSecsSinceEpoch(quote.regularMarketTime).toString());
                    }
                    else
                    {
                        ...
                        switch (error)
                        {
                            ...
                            case ParseError::MissingField:
                                qDebug() << "Error: Missing expected field in JSON response for stock data request for" << symbol;
                                break;
                        }

                        this->m_resultText = QStringLiteral("Error fetching stock data for %1: %2").arg(symbol).arg(static_cast<int>(error));
                    }
                }
                else if (statusCode.isValid())
                {
                    qDebug() << "HTTP error fetching stock data for" << symbol << ":" << statusCode.toInt();
                    this->m_resultText = QStringLiteral("HTTP error fetching stock data for %1: %2").arg(symbol).arg(statusCode.toInt());
                }
                else
                {
                    qDebug() << "No response received for stock data request for" << symbol << ":" << reply->errorString();
                    this->m_resultText = QStringLiteral("No response received for stock data request for %1: %2").arg(symbol).arg(reply->errorString());
                }
